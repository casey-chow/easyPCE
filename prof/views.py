...
def professor(request, netid):
    ...
    if thisprofessor is not None:
        try:
            allCourseNums = CourseNum.objects.filter(
                instance__profs__id=thisprofessor.id).distinct().order_by(
                'dept__dept', 'number')
            uniqueCourseGroups = []
            courseRegNums = []
            uniqueCourseNums = []
            for cn in allCourseNums:
                ...
                query = Course.objects.filter(
                    coursenum=cn, profs__id=thisprofessor.id).order_by(
                    '-year', '-semester')
            ...
            for i, uc in enumerate(uniqueCourseGroups):
                ...
                for cors in uc:
                    ...
                    if (int(year[:4]) < int(cors.year[:4])) or ((sem == 'Fall') and (
                            cors.semester == 'Spring') and (int(year[:4]) == int(cors.year[:4]))):
                        # Spring 2013-2014 Note. Must change to include Fall
                        # 2013.
                        if (int(cors.year[:4]) != 2013):

                            advice.insert(
                                i, Advice.objects.filter(
                                    instance=cors).order_by('length').reverse())
                            evaluation.insert(
                                i, Evaluation.objects.filter(
                                    instance=cors))
        ...
        try:
            departments = Department.objects.filter(
                professor=thisprofessor).order_by('name')
        ...
        c = Context({'user': user,
                     'alldepts': alldepts,
                     'prof': thisprofessor,
                     'unique': uniqueCourseNums,
                     'depts': departments,
                     'best': uniqueBestcourse,
                     'list': zipped})
