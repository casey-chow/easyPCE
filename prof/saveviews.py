...
#				uniquecourses.extend(CourseNum.objects.filter(instance=course))
            preuniquecourses = CourseNum.objects.filter(
            ...
            if uniquecourses:
                for c in uniquecourses:
                    query = Course.objects.filter(
                        coursenum=c, profs__id=thisprofessor.id).order_by(
                        '-year', '-semester')
                    if query:
        ...
        professor = []
    c = Context({'prof': thisprofessor, 'unique': uniquecourses,
                 'depts': departments, 'best': bestcourses})
    html = template.render(c)
    return HttpResponse(html)
