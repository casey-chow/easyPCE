...
def dephome(request, dept):
    ...
    if department is not None:
        ...
        try:
            precourses = CourseNum.objects.filter(
                dept__dept=dept).order_by('number')
            courses = []  # this precourses courses old courses junk handles sorting by next year
            oldCourses = []
            for c in precourses:
                sems = Course.objects.filter(
                    coursenum__id=c.id).order_by(
                    '-year', '-semester')
                if sems:
        ...
        if courses:
            profs = Professor.objects.filter(
                depts__dept=dept).order_by('lastname')
        else:
            profs = []
    else:
        url = None
    ...
    if courses:
        for i in range(len(courses) - 1):
            query = Course.objects.filter(
                coursenum__id=courses[i].id).order_by(
                '-year', '-semester')
