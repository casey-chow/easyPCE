...
class Curler:
    ...
    def login(self):
        ...
        c.setopt(
            c.URL,
            'https://puaccess.princeton.edu/psp/hsprod/EMPLOYEE/HRMS/h/?tab=DEFAULT')
        ...
        c.setopt(
            c.URL,
            'https://puaccess.princeton.edu/psc/hsprod/EMPLOYEE/HRMS/c/SA_LEARNER_SERVICES.SSS_STUDENT_CENTER.GBL')
        ...
        if match is None:
            raise Exception(
                "Failed to open course eval page. Could be a bad username/password?")

        applyweburl = match.group(0)
        applyweburl = applyweburl[1:-1]

        v("######################  ApplyWeb - browser check page  #####################")
        v("######################  from extracted applyweburl: " +
          applyweburl + " ########")
        ...
        if newurl is None:
            raise Exception(
                "Trouble parsing redirect url after browser check :(")
        ...
        c.setopt(
            c.URL,
            "https://www.applyweb.com/eval/EvalGatekeeper/EvalGatekeeper?service=external/RollupReportBrowser&random=1348716484361")
        c.setopt(c.HTTPGET, 1)
        response = self.gowait(c, False, True)
    ...
    def submit(self):
        ...
        for child in self.currentsoup.find_all('input'):
            if child.get('name') is None or child.get('value') is None:
        ...
        c.setopt(
            c.URL,
            "https://www.applyweb.com/eval/EvalGatekeeper/EvalGatekeeper")
    ...
    def recurse(self, index):
        ...
        if index >= len(self.selects):
            if self.fastforward:
                self.fastforward = False
                v("Found our spot!")
                # We've already saved this one (last run), so continue from
                # there!
                return True
            # Do stuff with the full form!
            coursenum = "%s%s" % (
                self.formvals['subjectSelect'], self.formvals['numberSelect'])
        ...
        if not self.fastforward:
            # restore the form values of fields BELOW the one we're iterating
            # on
            for i in range(index + 1, len(self.selects)):
                ...
                v("setting %s = %s (value string %s)" %
                  (field, option.get('value'), option.string))
        ...
        if select is None:
            ...
            raise Exception(
                "Error on page; couldn't find the form select tags")
        ...
        for option in select.contents:
            ...
            if re.match("All[\s]?.*", option.get('value')):
                continue
            v("setting %s = %s (value string %s)" %
              (nametext, option.get('value'), option.string))
    ...
    def saveData(self, html, coursenum, term):
        ...
        def isLink(element):
            if isinstance(
                    element,
                    Tag) and element.name == 'a' and element.string == "clicking here":
                return True
            return False
...
parser.add_argument(
    "maxdelay",
    help="the maximum number of seconds we will sleep between requests",
    type=int)
parser.add_argument(
    "-v",
    "--verbose",
