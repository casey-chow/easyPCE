#!/usr/bin/env python
"""
Modified by John Whelchel for COS333 Spring 2013
Modified by Casey Chow
...
termss['Fall_2015-2016'] = 1162
termss['Fall_2016-2017'] = 1172
...
def get_course_details(soup):
    ...
    return {
        'courseid': COURSE_URL_REGEX.search(
            soup.find(
                'a',
                href=COURSE_URL_REGEX)['href']).group('id'),
        'area': match.group(1) if match is not None else '',
        'title': clean(
            soup('h2')[1].string),
        'descrip': clean(
            descrdiv.contents[0] if descrdiv else ''),
        'prereqs': clean(
            pretitle.parent.findNextSibling(
                text=True)) if pretitle is not None else ''}
...
def get_course_profs(soup):
    ...
    return [{'uid': PROF_URL_REGEX.search(link['href']).group(
        'id'), 'name': clean(link.string)} for link in prof_links]
...
def get_course_classes(soup):
    ...
    # the first row is actually just column headings
    class_rows = soup('tr')[1:]
    ...
    return [get_single_class(row) for row in class_rows if row('td')[
        0].strong and row('td')[3].strong.string]


def scrape_page(page):
    """
    Returns a dict containing as much course info as possible
    from the HTML contained in page.
    """
...
def scrape_id(id):
    """Scrapes the information from a single course."""
    course_url = COURSE_URL.format(term=termss[TERM_CODE], courseid=id)
    print "\tscraping course id %s\n%s" % (id, course_url)
...
def scrape_all():
    ...
    """
    list_url = LIST_URL.format(term=termss[TERM_CODE])
    print "scraping courses from %s (%d)\n%s" % (TERM_CODE, termss[TERM_CODE], list_url)
    search_page = urllib2.urlopen(list_url)
    print "course list downloaded"
