...
#    print match
    ...
    return {
        'courseid': COURSE_URL_REGEX.search(
            soup.find(
                'a', href=COURSE_URL_REGEX)['href']).group('id'), 'title': clean(
            soup('h2')[1].string), 'descrip': clean(
                    descrdiv.contents[0] if descrdiv else ''), 'prereqs': clean(
                        pretitle.parent.findNextSibling(
                            text=True)) if pretitle is not None else '', 'pdf': clean(
                                soup.find('em').string)}
    ...
    for key in termss:
        for course in scrape_all(key):
            ...
            try:
                ...
                cn = CourseNum.objects.filter(
                    dept=d, number=course['listings'][0]['number'])
                sem = key.split("_")
                courses = Course.objects.filter(
                    semester=sem[0], year=sem[1], coursenum=cn)
                for c in courses:
                    c.lectureTime = None
                    c.preceptTime = None
                    if "Only" in course['pdf']:
                        ...
                        c.pdf = False
                    if ("npdf" in course['pdf']) or (
                            ("No" in course['pdf']) and not ("Audit" in course['pdf'])):
                        c.nopdf = True
                    else:
                        c.nopdf = False
                    ...
                    for i in course['classes']:
                        ...
                        if ("P" in i['type']) or ("B" in i['type']):
                            if c.preceptTime:
                                c.preceptTime = c.preceptTime + ";" + \
                                    i['days'] + ";" + i['starttime'] + "," + i['endtime']
                            else:
                                c.preceptTime = i['days'] + ";" + \
                                    i['starttime'] + "," + i['endtime']

                        else:
                            if c.lectureTime:
                                c.lectureTime = c.lectureTime + ";" + \
                                    i['days'] + ";" + i['starttime'] + "," + i['endtime']
                            else:
                                c.lectureTime = i['days'] + ";" + \
                                    i['starttime'] + "," + i['endtime']
                    c.save()
                    break
