...
def ranking(request):
    ...
    if not cns:
        cns = CourseNum.objects.exclude(
            bayes__isnull=True).order_by('-bayes')[:500]
        cache.set("BAYES_CNS", cns)
    cs = cache.get("BAYES_COURSES")
    if not cs:
        cs = []
        for cn in cns:
            c = Course.objects.filter(coursenum=cn).order_by(
                '-year', '-semester').only("regNum", "pdf", "nopdf", "da", "title")[:500]
            cs.append(c[0])
