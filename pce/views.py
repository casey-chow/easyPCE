...
def check_login(request, redirect):
    ...
    service_url = 'http://' + \
        urllib.quote(request.META['HTTP_HOST'] + request.META['PATH_INFO'])
    service_url = re.sub(r'ticket=[^&]*&?', '', service_url)
    service_url = re.sub(r'\?&?$|&$', '', service_url)
    if "ticket" in request.GET:
        val_url = cas_url + "validate?service=" + service_url + \
            '&ticket=' + urllib.quote(request.GET['ticket'])
...
def editfavorites(request):
	...
	"""
    try:
        nums = Course.objects.filter(
            regNum=regNum).order_by(
            '-year',
            '-semester')
    except:
        # course num parsing
...
def favorites(request):
    ...
    if favorites is not None:
        for i in range(len(favorites)):
            ...
            matches = Course.objects.filter(
                id=favorites[i].course.id).order_by(
                '-year', '-semester')
            try:
                if matches[
                        0].year == '2013-2014' and matches[0].semester == 'Spring':
    ...
    c = Context({'favorites': favorites, 'nextSemester': nextSemester,
                 'zip': zipped, 'alldepts': alldepts, 'user': user})
    return HttpResponse(t.render(c))
...
# Use this view with the accompanying URL to take the site down for
# maintenance.
...
def index(request):
    ...
    c = Context({'depts': depts, 'user': user,
                 'alldepts': alldepts, 'bayes': zipped})
    return HttpResponse(template.render(c))
...
def search(request):
    ...
    if 'q' in request.GET and request.GET['q']:
                ...
                try:
                    ...
                    if instances is not None and len(instances) > 0:
                        ...
                        while i < len(instances):
                            ...
                            while (
                                    i < len(instances) and instances[i].regNum == prev_r):
                                advice = Advice.objects.filter(instance=instances[i], text__icontains=text).exclude(
                                    text__icontains=("not"))  # can sort on length filed here, saves srting ~10 lines later???
                        ...
                        for c, al in sorted:
                            ...
                            numString = '<a href="/courses/%s%s">%s' % (
                                ccnums[0].dept.dept, ccnums[0].number, ccnums[0])
                            ...
                            for a in al:
                                ...
                                a.text = '%s<span id="highlight">%s</span>%s' % (
                                    a.text[0:f], a.text[f:f + len(text)], a.text[f + len(text):])
                        ...
                        html = t.render(
                            Context({'results': res, 'query': q, 'user': user, 'alldepts': ds}))
                        ...
                        for n in cnums:
                            ...
                            if instances is not None:
                                ...
                                for i in range(0, len(instances)):
                                    if i == 0:
                                        courseList.append(
                                            instances[i])  # change from i to zero (John 5/7/13 3 AM)?
                                    advice = Advice.objects.filter(instance=instances[i], text__icontains=text).exclude(
                                        text__icontains=("not"))  # can sort on length filed here, saves srting ~10 lines later???
                                    if advice is not None:
                                        for ad in advice:
        ...
        # Check for DEP123
        if (len(words) == 1 and re.search(
                "^[a-zA-Z]{3}[0-9]+.*$", words[0]) is not None):
        ...
        # Check for DEP 123
        if (
            len(words) == 2 and re.search(
                "^[a-zA-Z]{3}$",
                words[0]) is not None and re.search(
                "[0-9]+.*",
                words[1]) is not None):
            try:
                courses = Course.objects.filter(
        ...
        try:
            ...
            if len(courses) > 0:
                ...
                return HttpResponseRedirect(
                    "/courses/%s%s" %
                    (num.dept.dept, num.number))
        except:
            pass

        # Check for course name
        # in order to make this better, need to enable fulltext search by
        # changing database engine to MYISAM
        try:
            ...
            if len(courses) > 0:
                ...
                # result entry may not display latest info. should fix
                # eventually.
                links = []
                for i in range(0, len(unique)):
                    ...
                    dept = cnum.dept.dept
                    n = cnum.number
                    a = '<a href="/courses/%s%s">%s: %s</a>' % (
                        dept, n, numStrings[i], c.title)
                    links.append(a)
                results = results + "</ul>"
        ...
        try:
            ...
            profs = Professor.objects.filter(
                firstname=first, lastname=words[-1])
            if profs.count() > 0:
        ...
        try:
            ...
            if len(profs) > 0:
                pl = []
                for p in profs:
                    a = '<a href="/profs/%s">%s %s</a>' % (
                        p.netid, p.firstname, p.lastname)
                    pl.append(a)
        ...
        return HttpResponse(
            t.render(Context({'user': user, 'q': q, 'alldepts': all_depts})))

    all_depts = Department.objects.all().order_by('dept')
    t = get_template('search_empty.html')
    return HttpResponse(
        t.render(Context({'user': user, 'q': '', 'alldepts': all_depts})))
