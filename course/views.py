...
import _ssl
_ssl.PROTOCOL_SSLv23 = _ssl.PROTOCOL_SSLv3  # why is this here?
import urllib
import re
...
def course(request, subj, num):
    ...
    try:
        ...
        url = []
        favNum = []
        con = Context({'course': coursesWithUpcoming,
                       'list': zipped,
                       'advices': advice,
                       'evals': evaluation,
    ...
    if num is None:
        ...
        try:
            ...
            coursesWithUpcoming = Course.objects.filter(
            ...
            for c in coursesWithUpcoming:
                ...
                #	courses.append(c)
            termNum = termss[
                str(coursesWithUpcoming[0].semester) + " " + str(coursesWithUpcoming[0].year)]
    ...
    if courses is not None:
        if len(courses) > 0:
            coursenum = CourseNum.objects.filter(
                instance__regNum=courses[0].regNum)
    else:
